...
% Simple Training frrom Start
% Set trainNewModel as true
trainNewModel=true;

% Training from Saved checkpoint
% Set trainNewModel & checkpointing both True and set 'checkpointModelPath' path where checkpoint is present
% trainNewModel=true; checkpointing=false;

% For Bulk image Testing without training the model 
% Set trainNewModel=false and checkpointing=false; 
% Set 'savedModelPath' path for the trained detector.mat file
% Also Set 'testFile' path where training set path is their. See the "Test Phase" at the bottom.
% For using our trained model on single image call: "faceDetector(<yourimage>,DetectorType.WiderVgg16)"
